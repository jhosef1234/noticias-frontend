        ...
        <div *ngIf="mensajes.length === 0" class="flex items-start gap-2">
          ...
          <div class="flex-1 bg-white dark:bg-gray-700 rounded-2xl rounded-tl-none p-4 shadow-md">
            <p class="text-gray-800 dark:text-gray-200 text-sm">
              ¡Hola! 👋 Soy tu asistente virtual exclusivo para miembros Pro. Puedes escribirme cualquier pregunta sobre Noticias al Día y te ayudaré con información avanzada.
            </p>
          </div>
        </div>
...
<div style="text-align: center; padding: 20px 0;">
  ...
  private async procesarPregunta(pregunta: string): Promise<{ contenido: string; noticias?: NoticiaChatbot[] }> {
    ...
    if (this.contienePalabras(palabras, ['adiós', 'adios', 'gracias', 'chao', 'bye', 'hasta luego'])) {
      return { contenido: '¡De nada! 😊 Estoy aquí siempre que necesites ayuda. Si tienes más preguntas sobre Noticias al Día, no dudes en escribirme. ¡Que tengas un excelente día!' };
    }
